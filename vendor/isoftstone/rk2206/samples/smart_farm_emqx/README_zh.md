...
本例程演示如何在通晓开发板上基于华为云IOTDA平台实现物联网智慧家居应用案例。
...
![](../../docs/figures/smart_farm_hwiot/bh1750原理图.jpg)

![](../../docs/figures/smart_farm_hwiot/sht30原理图.jpg)

![](../../docs/figures/smart_farm_hwiot/电机原理图.jpg)

![](../../docs/figures/smart_farm_hwiot/rgb原理图.jpg)

![](../../docs/figures/smart_farm_hwiot/液晶屏原理图.jpg)

![](../../docs/figures/smart_farm_hwiot/语音模块原理图.jpg)
...
在`smart_farm_thread`线程中初始化lcd、电机、led、语音模块设备。通过接收队列传输的数据进行判断显示。
...
在`device_read_thraed`线程中初始化i2c传感器设备，将读取的数据放入队列中传输到`smart_farm_thread`与`su_03t_thread`线程中进行数据处理。
...
![](../../docs/figures/smart_farm_hwiot/语音模块-创建产品.jpg)

选`AI语音音响`-`纯离线方案`-`SU03-T模块`。

![](../../docs/figures/smart_farm_hwiot/语音模块-产品定义.jpg)

##### 生成语音内容

将21、22引脚修改为`UART1_TX`和`UART1_RX`。

![](../../docs/figures/smart_farm_hwiot/语音模块-修改串口.jpg)

修改唤醒词为`通晓通晓`

![](../../docs/figures/smart_farm_hwiot/语音模块-修改唤醒词.jpg)
...
![](../../docs/figures/smart_farm_hwiot/语音模块-配置基础信息.jpg)

配置命令词触发。以开灯为例。

![](../../docs/figures/smart_farm_hwiot/语音模块-命令词回复.jpg)

输出方式选择`端口输出`；控制类型选择`UART1_TX`；动作选择`发送`；参数`01 01`。

参数每个人都可以自定义自己的参数。

![](../../docs/figures/smart_farm_hwiot/语音模块-命令词触发.jpg)

串口输入触发。以获取湿度为例。

通过`命令词触发`语音模块向通晓开发板发送`03 02`指令。

![](../../docs/figures/smart_farm_hwiot/语音模块-获取湿度.jpg)

通晓开发板通过回复一定格式的串口数据，用于触发`HumiditySend`行为。`HumiditySend`行为触发后播报湿度。

![](../../docs/figures/smart_farm_hwiot/语音模块-获取湿度2.jpg)
...
##### 创建云平台项目
登录华为云[IOTDA平台](https://console.huaweicloud.com/iotdm/
)并开通*IoTDA实例*

点击`创建产品`。创建一个新产品。

![](../../docs/figures/smart_farm_hwiot/云平台-创建产品.jpg)

添加`smart_farm`服务。

添加所需要的属性和命令。

![](../../docs/figures/smart_farm_hwiot_hwiot/云平台-添加服务.jpg)

在`我的设备`中，选择`注册设备`进行设备注册。

![](../../docs/figures/smart_farm_hwiot/云平台-注册设备.jpg)
...
修改 `vendor/isoftstone/rk2206/sample` 路径下 BUILD.gn 文件，指定 `iot_smart_farm_example` 参与编译。

```r
"./smart_farm_emqx:iot_smart_farm_example",
```

修改 `device/rockchip/rk2206/sdk_liteos` 路径下 Makefile 文件，添加 `-liot_smart_farm_example` 参与编译。

```r
hardware_LIBS = -lhal_iothardware -lhardware -liot_smart_farm_example,
```
...
![](../../docs/figures/smart_farm_hwiot/云平台-显示.jpg)
